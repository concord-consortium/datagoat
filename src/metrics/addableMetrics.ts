...
// Verbatim port of the prototype's `addableHealthMetrics` /
// `addableCompetitionMetrics` arrays (datagoat-2026-04-27.html lines
...
function buildPlaceholders(
  ...
  for (let i = 1; i <= 10; i++) {
    out.push({
      id: `${type}-custom-${i}`,
      name: `${type === "health" ? "Health" : "Competition"} Metric${i}`,
      unit: "",
      type,
      whoCollects: "",
      howCollected: "",
...
export const ADDABLE_HEALTH: MetricDefinition[] = buildPlaceholders("health");
export const ADDABLE_COMPETITION: MetricDefinition[] =
  buildPlaceholders("competition");
