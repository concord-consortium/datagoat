...
const HEALTH_DATE = "2026-04-28";
const HEALTH_DATE_B = "2026-04-29";
...
describe("DataContext bi-state transitions", () => {
  it("starts in 'loading' for both kinds when no user is signed in", () => {
    ...
    expect(result.current.health.status).toBe("loading");
    expect(result.current.competition.status).toBe("loading");
  });

  it("transitions health loading -> loaded with empty entries", async () => {
    ...
    await waitFor(() => {
      ...
      if (result.current.health.status === "loaded") {
        expect(result.current.health.entries).toEqual([]);
      }
    });
  });

  it("transitions health loading -> loaded with migrated entries", async () => {
    state.user.current = { uid: "u1" };
    const { result } = renderHook(() => useData(), { wrapper });
    emit(latestSub(state.healthSubs), [
      {
    ...
    await waitFor(() => {
      expect(result.current.health.status).toBe("loaded");
      if (result.current.health.status === "loaded") {
        expect(result.current.health.entries.length).toBe(1);
        expect(result.current.health.entries[0].hydration).toBe(4);
      }
    });
  });

  it("health and competition load states are independent", async () => {
    state.user.current = { uid: "u1" };
    const { result } = renderHook(() => useData(), { wrapper });
    emit(latestSub(state.healthSubs), []);
    await waitFor(() => {
    ...
    await waitFor(() => {
      expect(result.current.competition.status).toBe("loaded");
    });
  });
});
...
function driveLoaded(
  ...
  emit(latestSub(state.competitionSubs), []);
  if (result.current.health.status !== "loaded") {
    throw new Error(`health not loaded after emit (uid=${uid})`);
  }
  if (result.current.competition.status !== "loaded") {
    throw new Error(`competition not loaded after emit (uid=${uid})`);
  }
}
...
describe("DataContext debounce accumulator (lifted from log components)", () => {
  ...
  it("multi-field typing within 500ms fires one merged setDoc", () => {
    ...
    act(() => {
      result.current.setHealthEntry(HEALTH_DATE, { sleepTime: 8 });
    });
    expect(state.setDoc).not.toHaveBeenCalled();
    act(() => {
      vi.advanceTimersByTime(500);
  ...
  it("typing on date A then date B flushes both independently (per-date timers)", () => {
    ...
    act(() => {
      result.current.setHealthEntry(HEALTH_DATE_B, { sleepTime: 8 });
    });
    act(() => {
      vi.advanceTimersByTime(500);
    });
    ...
    expect(calls).toContainEqual(
      expect.objectContaining({ hydration: 5, date: HEALTH_DATE }),
    );
    expect(calls).toContainEqual(
      expect.objectContaining({ sleepTime: 8, date: HEALTH_DATE_B }),
    );
  });
  ...
  it("provider unmount flushes all pending dates with their captured uid", () => {
    ...
    act(() => {
      result.current.setHealthEntry(HEALTH_DATE, { hydration: 5 });
      result.current.setHealthEntry(HEALTH_DATE_B, { hydration: 3 });
    });
    expect(state.setDoc).not.toHaveBeenCalled();
    unmount();
    expect(state.setDoc).toHaveBeenCalledTimes(2);
  ...
  it("rejects out-of-window, future, and malformed dates: no setDoc, no overlay, logError fires", () => {
    ...
    act(() => {
      result.current.setHealthEntry(farPastIso, { hydration: 5 });
      result.current.setHealthEntry(futureIso, { hydration: 5 });
      result.current.setHealthEntry("not-a-date", { hydration: 5 });
      result.current.setCompetitionEntry(farPastIso, {
        metrics: { goals: 1 },
      });
      result.current.setCompetitionEntry(futureIso, {
        metrics: { goals: 1 },
      });
      result.current.setCompetitionEntry("2026-13-40", {
        metrics: { goals: 1 },
      });
    });
    if (result.current.health.status !== "loaded") {
    ...
    expect(result.current.health.entries).toEqual([]);
    expect(result.current.competition.entries).toEqual([]);
    act(() => {
      vi.advanceTimersByTime(500);
    });
    expect(state.setDoc).not.toHaveBeenCalled();
  ...
  it("4d no-user no-op: setHealthEntry / setCompetitionEntry are no-ops when signed out", () => {
    state.user.current = null;
    const { result } = renderHook(() => useData(), { wrapper });
    expect(() => {
      act(() => {
        result.current.setHealthEntry(HEALTH_DATE, { hydration: 5 });
        result.current.setCompetitionEntry(HEALTH_DATE, {
          metrics: { goals: 3 },
        });
      });
    }).not.toThrow();
  ...
  it("4e user A -> user B switch: pending discarded, no setDoc, fresh subscribe", () => {
    ...
    expect(state.setDoc).not.toHaveBeenCalled();
    // health resets to loading until B's snapshot lands.
    expect(result.current.health.status).toBe("loading");
    emit(latestSub(state.healthSubs), [
      {
        path: `users/userB/healthEntries/${HEALTH_DATE}`,
        data: {
          version: 1,
          date: HEALTH_DATE,
          hydration: 1,
    ...
    expect(result.current.health.status).toBe("loaded");
    if (result.current.health.status === "loaded") {
      expect(result.current.health.entries[0].hydration).toBe(1);
    }
  });
  ...
  it("omits version stamp when server snapshot is already at CURRENT", () => {
    ...
    emit(latestSub(state.healthSubs), [
      {
        path: `users/u1/healthEntries/${HEALTH_DATE}`,
        data: {
          version: 1, // === CURRENT_HEALTH_ENTRY_VERSION
          date: HEALTH_DATE,
          hydration: 4,
          sleepTime: 8,
          sleepEfficiency: 85,
    ...
    emit(latestSub(state.competitionSubs), []);
    act(() => {
      result.current.setHealthEntry(HEALTH_DATE, { hydration: 6 });
    });
    act(() => {
      vi.advanceTimersByTime(500);
    });
    ...
    expect(payload.date).toBe(HEALTH_DATE);
...
describe("DataContext optimistic merge memo", () => {
  ...
  it("6 setHealthEntry exposes the partial synchronously before flush", () => {
    const { result } = renderHook(() => useData(), { wrapper });
    driveLoaded(result, "u1");
    act(() => {
      result.current.setHealthEntry(HEALTH_DATE, { hydration: 5 });
  ...
  it("6b competition optimistic merge preserves established metrics", () => {
    const { result } = renderHook(() => useData(), { wrapper });
    emit(latestSub(state.healthSubs), []);
    emit(latestSub(state.competitionSubs), [
      {
        path: `users/u1/competitionEntries/${HEALTH_DATE}`,
        data: {
          version: 1,
          date: HEALTH_DATE,
    ...
    const entry = result.current.competition.entries.find(
      (e) => e.date === HEALTH_DATE,
    );
