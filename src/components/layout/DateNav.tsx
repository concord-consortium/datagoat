...
export interface DateNavProps {
  // The current date offset in [0, HISTORY]. The parent derives this from
  // useSearchParams.
  offset: number;
...
// just work. Direct navigation to /log with no ?date= falls back
...
function chipStatusText(state: ChipState): string {
  // Category is contextually obvious — the chip lives on the Metrics
  // Data Entry Log, so no need to repeat the category descriptor here.
...
// Re-export so callers like MetricsDataEntryLog can convert their
