...
4. **Configure auth providers in the staging project:**
   ...
   - **Google OAuth** — toggle Google on in Firebase Auth → Sign-in method. Firebase auto-provisions a Web OAuth 2.0 Client in the staging GCP project, pre-wired for `https://datagoat-staging.firebaseapp.com/__/auth/handler` and `https://datagoat-staging.web.app/__/auth/handler`. No manual GCP console work is needed for the default flow, and no Client ID needs to be pasted into Firebase. (Reusing the prod Client ID across projects via a custom Web SDK config override is technically possible but not recommended — it crosses GCP project boundaries unnecessarily.)
   - **Facebook OAuth** — **recommended: register a new Facebook App for staging** in the Facebook developer console, with OAuth redirect URI `https://datagoat-staging.firebaseapp.com/__/auth/handler`. Keep it in Dev mode with test users while staging stabilizes. Paste the new App ID + secret into staging Firebase Auth. Hard-isolates staging traffic from the prod app's rate limits, anti-abuse signals, App Review status, and audit trail. **Alternative: reuse the prod Facebook App** — add the staging redirect URI to the existing prod app's Valid OAuth Redirect URIs and paste the same App ID + secret into staging Firebase Auth. Simpler setup; couples staging health to prod app standing (a staging mishap can affect prod sign-in).
