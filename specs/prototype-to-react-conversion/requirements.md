...
| dashboard-screen | Home / summary view | Dashboard header slides between wordmark and a streak/motivation message (7 rotating messages, name-substituted). Section calendar (heatmap-style: All / Some / None / inactive) for Health & Wellness activity (no Performance calendar - matches prototype HTML lines 4170-4225, deviation tracked in implementation.md "Dashboard scaffold"). **Health & Wellness calendar cells are interactive**: tapping/clicking (or pressing Enter/Space on focus) a non-inactive day navigates to the Health & Wellness Log on that date. Log-status buttons ("Log your X metrics for today") linking to each log screen. Two chart cards (Health & Wellness + Performance) with metric dropdown and time-range picker: 7d / 2w / 30d / 3mo / 6mo / All. "Analyze Your Data in CODAP" button. |
...
src/
  components/
    ...
    dashboard/
      ...
      ActivityCalendar.tsx        - section calendar (heatmap All/Some/None/inactive) for Health & Wellness + Performance. Health & Wellness cells are tappable when **both** filters pass: (a) `state !== inactive`, and (b) the cell's calculated date offset is in `[0, HISTORY]` (not before the first tracked day, not in the future). Tappable cells render as a plain `<Link to="/wellness?date=YYYY-MM-DD">` — no `role="button"`, no explicit `tabindex` (anchors are keyboard-focusable by default), no synthetic Space handler. Enter activates the link natively, which is the WCAG-correct behavior for navigation links (links activate on Enter; Space activation is button semantics). Preserving native anchor semantics also keeps Cmd/Ctrl/middle-click "open in new tab" and the hover URL preview working. Cells outside the active window render as a `<div>` with no role / tabindex / click handler. Performance cells remain non-interactive in this story.
      ...
      TimeRangePicker.tsx         - 7d / 2w / 30d / 3mo / 6mo / All pill picker for charts
...
interface AuthState {
  ...
  isEmailVerifiedOrTrusted: boolean; // user.emailVerified OR signed in via a trusted OAuth provider (Google, Facebook); see `isEmailVerifiedOrTrustedProvider` helper
}
```
...
- Once in the app, `VerificationBanner` shows a dismissible reminder if `!isEmailVerifiedOrTrusted && daysUnverified >= 7`
- `daysUnverified` is calculated **inside `VerificationBanner`** from `user.metadata.creationTime` vs current date. Both inputs are client-trusted (the creation time travels with the JWT and `Date.now()` is the local clock), so the banner is **advisory only** - a soft nudge, not a compliance gate. Real verification enforcement keys off `isEmailVerifiedOrTrustedProvider(user)` (see the CODAP plugin sign-out path below). The banner refreshes its computation hourly and on `visibilitychange` so a long-running PWA session crosses the 7-day threshold mid-session (without a refresh tick, the User reference is reused across token refreshes and AuthContext never re-renders consumers, so `Date.now()` is read once at mount and the threshold is never re-checked).
- **Trusted-provider OAuth (Google + Facebook) is treated as verified-equivalent.** The rule is "user has `emailVerified=true` OR signed in via a trusted OAuth provider (Google, Facebook) that returned an email" - encoded in `isEmailVerifiedOrTrustedProvider` (see [src/components/auth/authProviders.ts](src/components/auth/authProviders.ts)) and the single source of truth for routing, the verification banner, and the CODAP plugin gate. `sendEmailVerification` is not called for trusted-provider sign-ins and the banner never shows for them. The only `emailVerified=false` users hitting the verify-email flow are password signups. Rationale: Google and Facebook require email-ownership verification on their side before issuing an OAuth token that includes the email, so a successful OAuth with email present is equivalent proof of ownership - asking the user to also click a Firebase-sent link to the same address is friction with no security gain. The raw `user.emailVerified` flag is left untouched; Firebase still reports the truth, we just stop using it as a hard gate. Adding a third provider later is a one-line change to `TRUSTED_OAUTH_PROVIDERS`.
...
- `VerificationBanner` dismissible banner shown inside the app when `!isEmailVerifiedOrTrusted && daysUnverified >= 7` (so trusted-provider users - Google, Facebook - never see it; only password signups who haven't clicked the link).
- `sendPasswordResetEmail(auth, email)` for the Forgot Password flow. Success copy is **deliberately neutral on account existence**: *"If an account exists for that email, we sent a reset link. Check your inbox. If it doesn't appear within a few minutes, check your spam folder. If you've requested several resets recently, please wait a bit before trying again."* - shown whether or not the email is registered. This matches Firebase Auth's server-side behavior and avoids user enumeration. Do not display the email address back to the user or any variation of "Sent to you@school.edu." Errors that could leak account existence (`auth/user-not-found`, `auth/too-many-requests` — Firebase applies per-account rate-limiting on resets, so surfacing it would create an account-existence oracle) bucket into this same neutral confirm screen; only `auth/network-request-failed` is surfaced inline, since it is account-state-independent. The rate-limit sentence in the confirm copy gives a legitimately throttled user a non-leaking explanation.
- Password show/hide eye-toggle button (`.eye-btn` from the prototype) on password fields.
- `AuthContext` - wraps `useState(user)` + `onAuthStateChanged`, derives `isNewUser` (no Firestore profile yet) and `isEmailVerifiedOrTrusted` (delegates to `isEmailVerifiedOrTrustedProvider`). Provides a `signOut` helper. (`daysUnverified` is **not** on the context — it's computed inside `VerificationBanner` on each render so a long-running PWA session can cross the 7-day threshold mid-session; see the Email verification flow above.)
...
- **Inline account-linking flow** when `auth/account-exists-with-different-credential` fires (a Facebook user whose email is already registered under another provider). The LoginForm / SignupForm catches the error, extracts the pending Facebook credential via `FacebookAuthProvider.credentialFromError(error)`, and flips the screen to a linking-mode view (component-local state — no separate route). The linking view tells the user "This email is already registered with DataGOAT. Sign in with the method you used originally to link Facebook to your account." and renders **both** a "Continue with Google" button **and** an email + password form (email locked); the user picks the method they used. On successful sign-in with the existing provider, `linkWithCredential(result.user, pendingCredential)` is called (using the user from the just-resolved sign-in promise — not `auth.currentUser` — so the linkage is scoped to the user that authenticated for this flow and isn't subject to cross-tab races) and the user is navigated to `/dashboard` with both providers linked. The pending credential is held in component-local state — refreshing during the flow restarts it. Profile-screen "Linked Accounts" management UI is **out of scope** (sign-in-time linking only). Reference: https://firebase.google.com/docs/auth/web/account-linking.

  **Deviation from earlier spec**: an earlier draft called `fetchSignInMethodsForEmail(email)` to render the existing provider's name and only that provider's sign-in surface. That call leaks account existence and provider to an unauthenticated client, defeating ForgotPassword's enumeration-resistant copy; Google has also deprecated the API for this reason. The collision flow is now method-agnostic: we always show both Google and email/password options and let the user pick.
...
- **Trusted-provider verification short-circuit**: skip `sendEmailVerification` and the verification banner whenever `isEmailVerifiedOrTrustedProvider(user)` is true - i.e., either `user.emailVerified === true` or the user signed in via a trusted OAuth provider (Google, Facebook) that returned an email. The helper is the single source of truth used by routing, the banner, and the CODAP plugin gate. Adding more providers later (Apple, Microsoft, etc.) is a one-line change to `TRUSTED_OAUTH_PROVIDERS` in `authProviders.ts`.
...
The `/codap` route is **not** wrapped in `ProtectedRoute`. `CodapPlugin` inspects `AuthContext.user` directly and branches three ways: unauthenticated renders `<CodapPluginSignIn>`, authenticated-but-not-verified-or-trusted (i.e., a password user with `emailVerified=false`) renders a "verify your email at datagoat.concord.org" notice plus a sign-out button, and verified-or-trusted-provider renders the data-export UI - matching the auth behavior described in the "Auth in iframe" subsection below.
...
After sign-in, if `isEmailVerifiedOrTrustedProvider(user)` is false (i.e., a password user who hasn't clicked the verification link yet), the plugin signs the user back out and shows a notice pointing to the main DataGOAT app for verification. This matches `LoginForm`'s `/verify-email` redirect; the plugin enforces it via sign-out instead of routing because it has no router. Trusted-provider sign-ins (Google, Facebook with email) bypass this gate the same way they bypass routing in the main app.
...
- **Firestore-persistence code paths beyond the contexts described in this spec**: tracked-metric order persistence, per-day availability defaults - follow-up stories. Note: the offline write queue is **in scope** (see Firebase init below) — DataGOAT is a PWA where users may log on the sideline with no connection, so `initializeFirestore` enables `persistentLocalCache({ tabManager: persistentMultipleTabManager() })`. Without that, offline writes queue only in memory and are lost on tab close.
...
### Designer-final addable-metric set

- **What's deferred**: the AddMetric screen's catalog of additional metrics users can add to their tracked set (beyond the default `WELLNESS_METRICS` / `PERFORMANCE_METRICS` lists).
- **What ships now**: a verbatim port of the prototype's own placeholder data — a 10-iteration loop per type generating `wellness-custom-1..10` / `performance-custom-1..10` with names `"Wellness Metric1..10"` / `"Performance Metric1..10"` and no info-modal copy (the prototype source at datagoat-2026-04-27.html lines 8180-8185 carries only `id` and `name`). The `ADDABLE_WELLNESS` / `ADDABLE_PERFORMANCE` registries in `src/metrics/addableMetrics.ts` are the seam; the AddMetric screen reads from them, never from a hardcoded list.
- **Acceptance for the follow-up**: the placeholder loop is replaced with a designer-final list of real addable metrics (real `name`, `whoCollects`, `howCollected`, `description`, `unit`, `inputType` per entry); **only `addableMetrics.ts` changes**; AddMetric screen, MetricDetail info-modal wiring, and consumer call sites are unchanged.
...
- [ ] Dashboard includes: header slide + motivation carousel, wellness activity calendar, log-status CTA buttons, two chart cards with time-range picker (7d / 2w / 30d / 3mo / 6mo / All), "Analyze Your Data in CODAP" button. **Charts inside the cards are placeholder gray-box components** at the final dimensions accepting the full final prop surface; real SVG chart rendering is a follow-up story (per Decisions: Chart library); performance section omits its calendar per prototype HTML 4170-4225
...
The Third pass resolved `auth/account-exists-with-different-credential` with a friendly error message ("Sign in with your existing method, then link Facebook from your profile") and treated the linking flow itself as out of scope. The deferred message references a Profile-screen flow that doesn't exist yet, leaving users with no way to actually link providers. **Resolution**: implement the inline linking flow now, sign-in-time only. When `auth/account-exists-with-different-credential` fires, LoginForm/SignupForm catches the error, extracts the pending Facebook credential via `FacebookAuthProvider.credentialFromError(error)`, and flips the screen to a linking-mode view (component-local state — no separate route). The linking view tells the user "This email is already registered with DataGOAT. Sign in with the method you used originally to link Facebook to your account." and renders both a Google button and an email/password form (email locked); the user picks the method they used. On success, `linkWithCredential(currentUser, pendingCredential)` attaches Facebook to the existing account and the user lands on `/dashboard`. Pending credential lives in component-local state (not sessionStorage); refreshing during the flow restarts it. **Out of scope**: a Profile-screen "Linked Accounts" section for after-the-fact provider management — sign-in-time linking only. Reference: https://firebase.google.com/docs/auth/web/account-linking. The Third pass entry "Facebook account-collision UX not specified" is marked superseded.

**Subsequent revision (post-implementation security review)**: an earlier draft called `fetchSignInMethodsForEmail(email)` to pick which sign-in surface to show. That call leaks account existence + provider to an unauthenticated client and Google has deprecated it for that reason. We now omit the lookup and show both options unconditionally; the user picks the method they used originally.
