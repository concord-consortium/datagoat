...
- `AuthContext` - wraps `useState(user)` + `onAuthStateChanged`, derives `isNewUser` (no Firestore profile yet) and `isEmailVerified`. Provides a `signOut` helper. (`daysUnverified` is **not** on the context — it's computed inside `VerificationBanner` on each render so a long-running PWA session can cross the 7-day threshold mid-session; see the Email verification flow above.)
